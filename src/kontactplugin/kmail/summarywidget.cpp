...
SummaryWidget::SummaryWidget(KontactInterface::Plugin* plugin, QWidget* parent)
    : KontactInterface::Summary(parent), mPlugin(plugin), mChangeRecorder(new Akonadi::ChangeRecorder(this))
{
    ...
    connect(mModel, &Akonadi::EntityTreeModel::error, this, [this](const QString& message) {
        mErrorLabel->setText(i18n("Error occurred: %1", message));
        errorOccurred();
    });
    ...
    connect(mChangeRecorder, qOverload<const Akonadi::Collection&>(&Akonadi::ChangeRecorder::collectionChanged), this,
            &SummaryWidget::slotCollectionChanged);
    connect(mChangeRecorder, &Akonadi::ChangeRecorder::collectionRemoved, this, &SummaryWidget::slotCollectionChanged);
    connect(mChangeRecorder, &Akonadi::ChangeRecorder::collectionStatisticsChanged, this,
            &SummaryWidget::slotCollectionChanged);
...
void SummaryWidget::displayModel(const QModelIndex& parent, int& counter, const bool showFolderPaths,
        ...
        if (col.isValid()) {
            ...
            if (((stats.unreadCount()) != Q_INT64_C(0)) && showCollection) {
                ...
                urlLabel->setToolTip(i18n("<qt><b>%1</b>"
                                          "<br/>Total: %2<br/>"
                                          "Unread: %3</qt>",
                                          col.name(), stats.count(), stats.unreadCount()));

                connect(urlLabel, SIGNAL(leftClickedUrl(QString)), this, SLOT(selectFolder()));
                urlLabel->setProperty("folderUrl", urlLabel->url());

                // Read and unread count.
                auto label = new QLabel(i18nc("%1: number of unread messages "
                                              "%2: total number of messages",
                                              "%1 / %2", stats.unreadCount(), stats.count()),
                                        this);
...
void SummaryWidget::slotUpdateFolderList()
{
    ...
    int counter = 0;
    qCDebug(KMAILPLUGIN_LOG) << QStringLiteral("Iterating over") << mModel->rowCount()

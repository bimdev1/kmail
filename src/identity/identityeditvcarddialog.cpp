...
IdentityEditVcardDialog::IdentityEditVcardDialog(const QString& fileName, QWidget* parent)
    : QDialog(parent), mContactEditor(new Akonadi::AkonadiContactEditor(Akonadi::AkonadiContactEditor::CreateMode,
                                                                        Akonadi::AkonadiContactEditor::VCardMode, this))
{
...
void IdentityEditVcardDialog::slotDeleteCurrentVCard()
{
    ...
    const int answer = KMessageBox::questionTwoActions(this, i18n("Are you sure you want to delete this vCard?"),
                                                       i18nc("@title:window", "Delete vCard"), KStandardGuiItem::del(),
...
void IdentityEditVcardDialog::deleteCurrentVcard(bool deleteOnDisk)
{
    if (!mVcardFileName.isEmpty()) {
        if (deleteOnDisk) {
            ...
            if (file.exists()) {
                if (!file.remove()) {
                    KMessageBox::error(this, i18n("We cannot delete vCard file."),
...
void IdentityEditVcardDialog::reject()
{
    const int answer = KMessageBox::questionTwoActions(
        this, i18nc("@info", "Do you really want to cancel?"), i18nc("@title:window", "Confirmation"),

...
#endif
KActionMenuTransport::KActionMenuTransport(QObject* parent) : KActionMenu(parent)
{
    setPopupMode(QToolButton::DelayedPopup);
    connect(MailTransport::TransportManager::self(), &MailTransport::TransportManager::transportsChanged, this,
...
void KActionMenuTransport::setTransportActivitiesAbstract(TransportActivities* activities)
{
    ...
    connect(mTransportActivities, &TransportActivities::activitiesChanged, this,
            &KActionMenuTransport::forceUpdateTransportMenu);
}
#endif
    ...
    if (mInitialized) {
        ...
        for (MailTransport::Transport* transport : transports) {
            ...
            if (mTransportActivities) {
                if (mTransportActivities->hasActivitySupport() &&
                    !mTransportActivities->filterAcceptsRow(transport->activities())) {
                    continue;
                }
            }
#endif

...
#endif
KActionMenuAccount::KActionMenuAccount(QObject* parent) : KActionMenu(parent)
{
    ...
    connect(Akonadi::AgentManager::self(), &Akonadi::AgentManager::instanceNameChanged, this,
            &KActionMenuAccount::forceUpdateAccountMenu);
    connect(Akonadi::AgentManager::self(), &Akonadi::AgentManager::instanceRemoved, this,
            &KActionMenuAccount::forceUpdateAccountMenu);
    connect(Akonadi::AgentManager::self(), &Akonadi::AgentManager::instanceAdded, this,
            &KActionMenuAccount::forceUpdateAccountMenu);
}
...
void KActionMenuAccount::setAccountActivitiesAbstract(AccountActivities* activities)
{
    ...
    connect(mAccountActivities, &AccountActivities::activitiesChanged, this,
            &KActionMenuAccount::forceUpdateAccountMenu);
}
#endif
...
void KActionMenuAccount::updateAccountMenu()
{
    if (mInitialized) {
        ...
        for (const Akonadi::AgentInstance& type : lst) {
            ...
            const AgentIdentifier id(identifierName,
                                     QString(type.name()).replace(QLatin1Char('&'), QStringLiteral("&&")), index);

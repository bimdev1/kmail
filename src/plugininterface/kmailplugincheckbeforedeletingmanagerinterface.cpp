...
void KMailPluginCheckBeforeDeletingManagerInterface::initializePlugins()
{
    if (mWasInitialized) {
        qCDebug(KMAIL_LOG)
            << "KMailPluginCheckBeforeDeletingManagerInterface : Plugin was already initialized. This is a bug";
        return;
    }
    if (!mParentWidget) {
    ...
    for (MessageViewer::MessageViewerCheckBeforeDeletingPlugin* plugin : lstPlugin) {
        if (plugin->isEnabled()) {
            auto interface =
                static_cast<MessageViewer::MessageViewerCheckBeforeDeletingInterface*>(plugin->createInterface(this));
...
Akonadi::Item::List
KMailPluginCheckBeforeDeletingManagerInterface::confirmBeforeDeleting(const Akonadi::Item::List& list)
{

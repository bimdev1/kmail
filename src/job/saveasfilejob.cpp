...
SaveAsFileJob::SaveAsFileJob(QObject* parent) : QObject(parent) {}
...
void SaveAsFileJob::start()
{
    ...
    QStringList lst;
    if (mHtmlMode) {
        lst << QStringLiteral("text/html") << QStringLiteral("text/plain")
    ...
    if (dlg->exec()) {
        ...
        } else if (dlg->selectedNameFilter() == "application/vnd.oasis.opendocument.text"_L1 ||
                   filename.endsWith(".odf"_L1)) {

...
void LocalAIService::generateReply(const QString &emailContent, const QString &prompt)
{
    ...
    requestData["messages"] = QJsonArray{
        QJsonObject{
            {"role", "system"},
            {"content", "You are an email assistant. Generate a professional reply based on the email content and prompt."}
        },
        QJsonObject{
            {"role", "user"},
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, callback]() {
        ...
        if (reply->error() == QNetworkReply::NoError) {
            ...
            if (response.isObject() && response.object().contains("choices")) {
                QString content = response.object()["choices"].toArray().first()
...
void LocalAIService::handleError(QNetworkReply *reply)
{
    ...
    m_pendingRetry = {
        ...
        reply->request().rawHeader("Content-Type"),
        reply->readAll()
    };

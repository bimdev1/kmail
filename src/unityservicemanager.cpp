...
UnityServiceManager::UnityServiceManager(QObject* parent)
    : QObject(parent), mUnityServiceWatcher(new QDBusServiceWatcher(this))
{
    connect(kmkernel->folderCollectionMonitor(), &Akonadi::Monitor::collectionStatisticsChanged, this,
            &UnityServiceManager::slotCollectionStatisticsChanged);

    connect(kmkernel->folderCollectionMonitor(), &Akonadi::Monitor::collectionAdded, this,
            &UnityServiceManager::initListOfCollection);
    connect(kmkernel->folderCollectionMonitor(), &Akonadi::Monitor::collectionRemoved, this,
            &UnityServiceManager::initListOfCollection);
    connect(kmkernel->folderCollectionMonitor(), &Akonadi::Monitor::collectionSubscribed, this,
            &UnityServiceManager::initListOfCollection);
    connect(kmkernel->folderCollectionMonitor(), &Akonadi::Monitor::collectionUnsubscribed, this,
            &UnityServiceManager::initListOfCollection);
...
bool UnityServiceManager::excludeFolder(const Akonadi::Collection& collection) const
{
    ...
    if (CommonKernel->outboxCollectionFolder() == collection || CommonKernel->sentCollectionFolder() == collection ||
        ...
        CommonKernel->trashCollectionFolder() == collection || CommonKernel->draftsCollectionFolder() == collection ||
        CommonKernel->spamsCollectionFolder() == collection) {
        return true;
    }
...
void UnityServiceManager::slotCollectionStatisticsChanged(Akonadi::Collection::Id id,
...
void UnityServiceManager::updateCount()
{
    ...
    if (mUnityServiceAvailable) {
        ...
        const QVariantMap properties{{QStringLiteral("count-visible"), unreadEmail > 0},
                                     {QStringLiteral("count"), unreadEmail}};

        QDBusMessage message =
            QDBusMessage::createSignal(QStringLiteral("/org/kmail2/UnityLauncher"),
...
void UnityServiceManager::initUnity()
{
    ...
    mUnityServiceWatcher->setWatchMode(QDBusServiceWatcher::WatchForUnregistration |

...
GpgME::Key createTestKey(QByteArray uid, QByteArray fingerprint, time_t expires = 0,
                         ...
                         GpgME::UserID::Validity validity = GpgME::UserID::Full)
{
    ...
    subkey->can_encrypt =
        int(usage == Kleo::KeyCache::KeyUsage::AnyUsage || usage == Kleo::KeyCache::KeyUsage::Encrypt);
...
KMime::Message::Ptr createItem(const KIdentityManagementCore::Identity& ident,
                               QByteArray recipient = "Friends <friends@kde.example>")
{
    QByteArray data = "From: Konqui <konqui@kde.example>\n"
                      "To: " +
                      recipient +
                      "\n"
...
KMComposerWinTest::KMComposerWinTest(QObject* parent) : QObject(parent)
{
    ...
    // We need to initialize KMKernel after modifying the environment variables, otherwise we would read the wrong
    // configs
...
void KMComposerWinTest::initTestCase()
{
    ...
    Kleo::KeyCache::mutableInstance()->setKeys(
        {createTestKey("encrypt <encrypt@test.example>", "345678901"),
         createTestKey("wrongkey <wrongkey@test.example>", "22222222"), createTestKey("friends@kde.example", "1"),
         ...
         createTestKey("level0 <level0@tofu.example>", "023DFCA4424EA644B174AD14C6F20F3A31F563CF", 0,
                       GpgME::UnknownProtocol, Kleo::KeyCache::KeyUsage::AnyUsage, GpgME::UserID::Marginal),
         createTestKey("level1 <level1@tofu.example>", "B571A07AF0CE7BC645C98DDA3D081FB0E3ADDA15", 0,
                       GpgME::UnknownProtocol, Kleo::KeyCache::KeyUsage::AnyUsage, GpgME::UserID::Marginal),
         createTestKey("level2 <level2@tofu.example>", "C6359BFCBC418D756D52B9D095FC1B2659BD3F38", 0,
                       GpgME::UnknownProtocol, Kleo::KeyCache::KeyUsage::AnyUsage, GpgME::UserID::Marginal),
         createTestKey("level3 <level3@tofu.example>", "5FB1048166DF03F6AA15D57762EF1B9108E04EC9", 0,
                       GpgME::UnknownProtocol, Kleo::KeyCache::KeyUsage::AnyUsage, GpgME::UserID::Marginal),
         createTestKey("bad <bad@tofu.example>", "EF86281B34F05348610D5A6F3579DB31A5384419", 0, GpgME::UnknownProtocol,
                       Kleo::KeyCache::KeyUsage::AnyUsage, GpgME::UserID::Marginal),
         createTestKey("validity0@kde.example", "20", 0, GpgME::UnknownProtocol, Kleo::KeyCache::KeyUsage::AnyUsage,
                       GpgME::UserID::Unknown),
         createTestKey("validity1@kde.example", "21", 0, GpgME::UnknownProtocol, Kleo::KeyCache::KeyUsage::AnyUsage,
                       GpgME::UserID::Undefined),
         createTestKey("validity2@kde.example", "22", 0, GpgME::UnknownProtocol, Kleo::KeyCache::KeyUsage::AnyUsage,
                       GpgME::UserID::Never),
         createTestKey("validity3@kde.example", "23", 0, GpgME::UnknownProtocol, Kleo::KeyCache::KeyUsage::AnyUsage,
                       GpgME::UserID::Marginal),
         createTestKey("validity4@kde.example", "24", 0, GpgME::UnknownProtocol, Kleo::KeyCache::KeyUsage::AnyUsage,
                       GpgME::UserID::Full),
         createTestKey("validity5@kde.example", "25", 0, GpgME::UnknownProtocol, Kleo::KeyCache::KeyUsage::AnyUsage,
                       GpgME::UserID::Ultimate)});
}
...
void KMComposerWinTest::testSigning_data()
{
    ...
    QTest::newRow("encryptonly") << im->identityForAddress(QStringLiteral("encryptonly@test.example")).uoid() << false
                                 << true;
    QTest::newRow("signandencrypt") << im->identityForAddress(QStringLiteral("signandencrypt@test.example")).uoid()
                                    << true << true;
...
void KMComposerWinTest::testEncryption_data()
{
    ...
    QTest::newRow("nothing") << im->identityForAddress(QStringLiteral("nothing@test.example")).uoid() << recipient
                             << false << true;
    QTest::newRow("signonly") << im->identityForAddress(QStringLiteral("signonly@test.example")).uoid() << recipient
                              << false << true;
    QTest::newRow("encryptonly") << im->identityForAddress(QStringLiteral("encryptonly@test.example")).uoid()
                                 << recipient << true << true;
    QTest::newRow("signandencrypt") << im->identityForAddress(QStringLiteral("signandencrypt@test.example")).uoid()
                                    << recipient << true << true;
    QTest::newRow("autocrypt") << im->identityForAddress(QStringLiteral("autocrypt@test.example")).uoid()
                               << "Autocrypt <friends@autocrypt.example>" << true << true;
    QTest::newRow("wrongkey") << im->identityForAddress(QStringLiteral("wrongkey@test.example")).uoid() << recipient
                              << false << false;
    QTest::newRow("wrongkeysign") << im->identityForAddress(QStringLiteral("wrongkeysign@test.example")).uoid()
                                  << recipient << true << true;
}
...
void KMComposerWinTest::testEncryption()
{
    ...
    QVERIFY(file1.copy(
        autocryptDir.filePath(addrSpec.replace(QStringLiteral("@"), QStringLiteral("%40")) + QStringLiteral(".json"))));
    ...
    if (!instance->initialized()) {
        QEventLoop loop;
        connect(instance.get(), &Kleo::KeyCache::keyListingDone, this, [&loop]() { loop.quit(); });
        loop.exec();
    }
    QCoreApplication::processEvents(QEventLoop::AllEvents);
...
void KMComposerWinTest::testNearExpiryWarningIdentity_data()
{
    ...
    QTest::newRow("nothing") << im->identityForAddress(QStringLiteral("nothing@test.example")).uoid() << false
                             << QString();
    QTest::newRow("signonly") << im->identityForAddress(QStringLiteral("signonly@test.example")).uoid() << false
                              << QString();
    QTest::newRow("encryptonly") << im->identityForAddress(QStringLiteral("encryptonly@test.example")).uoid() << false
                                 << QString();
    QTest::newRow("signandencrypt") << im->identityForAddress(QStringLiteral("signandencrypt@test.example")).uoid()
                                    << false << QString();
    QTest::newRow("autocrypt") << im->identityForAddress(QStringLiteral("autocrypt@test.example")).uoid() << false
                               << QString();
...
void KMComposerWinTest::testRecipientAnnotation_data()
{
    ...
    QTest::newRow("friends@kde.example") << QStringLiteral("friends@kde.example") << Kleo::Level3 << QString()
                                         << QStringLiteral("emblem-success") << true;
    QTest::newRow("validity0@kde.example")
        << QStringLiteral("validity0@kde.example") << Kleo::Level0 << QStringLiteral("It hasn't enough validity.")
        << QStringLiteral("emblem-error") << false;
    QTest::newRow("validity1@kde.example")
        << QStringLiteral("validity1@kde.example") << Kleo::Level0 << QStringLiteral("It hasn't enough validity.")
        << QStringLiteral("emblem-error") << false;
    QTest::newRow("validity2@kde.example")
        << QStringLiteral("validity2@kde.example") << Kleo::Level0 << QStringLiteral("It hasn't enough validity.")
        << QStringLiteral("emblem-error") << false;
    QTest::newRow("validity3@kde.example")
        << QStringLiteral("validity3@kde.example") << Kleo::Level2
        << QStringLiteral("You can sign the key, if you communicated") << QStringLiteral("emblem-success") << true;
    QTest::newRow("validity4@kde.example") << QStringLiteral("validity4@kde.example") << Kleo::Level3 << QString()
                                           << QStringLiteral("emblem-success") << true;
    QTest::newRow("validity5@kde.example") << QStringLiteral("validity5@kde.example") << Kleo::Level4 << QString()
                                           << QStringLiteral("emblem-success") << true;
    QTest::newRow("level0@tofu.example") << QStringLiteral("level0@tofu.example") << Kleo::Level0
                                         << QStringLiteral("By using the key will be trusted more.")
                                         << QStringLiteral("emblem-warning") << true;
    QTest::newRow("level1@tofu.example") << QStringLiteral("level1@tofu.example") << Kleo::Level1
                                         << QStringLiteral("By using the key will be trusted more.")
                                         << QStringLiteral("emblem-success") << true;
    QTest::newRow("level2@tofu.example") << QStringLiteral("level2@tofu.example") << Kleo::Level2
                                         << QStringLiteral("By using the key will be trusted more.")
                                         << QStringLiteral("emblem-success") << true;
    QTest::newRow("level3@tofu.example") << QStringLiteral("level3@tofu.example") << Kleo::Level2
                                         << QStringLiteral("By using the key will be trusted more.")
                                         << QStringLiteral("emblem-success") << true;
    QTest::newRow("bad@tofu.example") << QStringLiteral("bad@tofu.example") << Kleo::Level0
                                      << QStringLiteral("The key is marked as bad.") << QStringLiteral("emblem-error")
                                      << false;
}

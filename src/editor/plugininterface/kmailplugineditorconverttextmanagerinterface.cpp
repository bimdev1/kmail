...
void KMailPluginEditorConvertTextManagerInterface::setInitialData(
    const MessageComposer::PluginEditorConverterInitialData& data)
{
    for (MessageComposer::PluginEditorConvertTextInterface* interface : std::as_const(mListPluginInterface)) {
...
void KMailPluginEditorConvertTextManagerInterface::setDataBeforeConvertingText(
    const MessageComposer::PluginEditorConverterBeforeConvertingData& data)
{
    for (MessageComposer::PluginEditorConvertTextInterface* interface : std::as_const(mListPluginInterface)) {
...
void KMailPluginEditorConvertTextManagerInterface::initializePlugins()
{
    if (!mListPluginInterface.isEmpty()) {
        qCWarning(KMAIL_LOG)
            << "KMailPluginEditorConvertTextManagerInterface : Plugin was already initialized. This is a bug";
        return;
    }
    if (!mRichTextEditor) {
    ...
    const QList<MessageComposer::PluginEditorConvertText*> lstPlugin =
        MessageComposer::PluginEditorConvertTextManager::self()->pluginsList();
    ...
    qCDebug(KMAIL_LOG) << "KMailPluginEditorConvertTextManagerInterface : Initialize done, number of plugins found: "
                       << mListPluginInterface.count();
}
...
QHash<MessageComposer::PluginActionType::Type, QList<QAction*>>
KMailPluginEditorConvertTextManagerInterface::actionsType()
{

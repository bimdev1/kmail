...
SettingsDialog::SettingsDialog(const KSharedConfigPtr& config, UnifiedMailboxManager& boxManager, WId, QWidget* parent)
    : QDialog(parent), mBoxModel(new QStandardItemModel(this)), mBoxManager(boxManager),
      mKernel(new MailKernel(config, this)), mConfig(config)
{
    ...
    h->addLayout(v);
    auto addButton =
        new QPushButton(QIcon::fromTheme(QStringLiteral("list-add-symbolic")), i18nc("@action:button", "Add"), this);
    ...
    auto editButton =
        new QPushButton(QIcon::fromTheme(QStringLiteral("entry-edit")), i18nc("@action:button", "Modify"), this);
    ...
    const auto removeMailBox = [this, view]() {
        ...
        if (!indexes.isEmpty()) {
            ...
            if (KMessageBox::warningTwoActions(
                    ...
                    i18nc("@title:window", "Really Remove?"), KStandardGuiItem::remove(),
    ...
    connect(view, &QListView::customContextMenuRequested, this,
            [this, view, addMailBox, removeMailBox, modifyMailBox](const QPoint& pos) {
                Q_UNUSED(pos);
                const auto mboxSelected = view->selectionModel()->selectedIndexes();
                QMenu menu(this);
                QAction* addAction =
                    menu.addAction(QIcon::fromTheme(QStringLiteral("list-add-symbolic")), i18nc("@action", "Add"));
                QAction* removeAction = nullptr;
                QAction* editAction = nullptr;
                if (!mboxSelected.isEmpty()) {
                    editAction =
                        menu.addAction(QIcon::fromTheme(QStringLiteral("entry-edit")), i18nc("@action", "Modify"));
                    menu.addSeparator();
                    removeAction = menu.addAction(QIcon::fromTheme(QStringLiteral("list-remove-symbolic")),
                                                  i18nc("@action", "Remove"));
                }
